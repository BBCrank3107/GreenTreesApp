...
app.post('/purchase', (req, res) => {
    const { name, email, phone, address, city, district, ward, ProductName, Price, Quantity, Image, UserID } = req.body;
    ...
    const insertSql = 'INSERT INTO seller (UserName, UserNumber, UserAddress, UserEmail, DateOrder, TimeOrder, ProductName, Price, Quantity, Image, Status, OrderCode, UserID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    db.query(insertSql, [name, phone, formattedAddress, email, currentDate, currentTime, ProductName, Price, Quantity, Image, 'Đang xử lý', orderCode, UserID], (insertErr, insertResult) => {
        if (insertErr) {
...
app.get('/api/orders/total/:orderCode', (req, res) => {
    ...
    db.query(totalPaymentSql, [orderCode], (err, results) => {
        if (err) {
            res.status(500).send({ success: false, message: 'Failed to retrieve total payment' });
            throw err;
        }
        if (results.length > 0) {
            res.status(200).send({ success: true, totalPayment: results[0].TotalPayment});
        } else {
